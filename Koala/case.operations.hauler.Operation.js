class HaulerOperation
{
    constructor(pRoom)
    {
        ...
        this.mContainers = _.filter(PCache.getEntityCache(ENTITY_TYPES.container), (aBox) => aBox.pos.roomName == this.mRoom.name);
        this.mStorage = _.find(PCache.getEntityCache(ENTITY_TYPES.storage), (aStorage) => aStorage.pos.roomName == this.mRoom.name);
        this.mSources = _.filter(PCache.getEntityCache(ENTITY_TYPES.source), (aSource) => aSource.pos.roomName == this.mRoom.name);
        this.mMineral = _.filter(PCache.getEntityCache(ENTITY_TYPES.mineral), (aSource) => aSource.pos.roomName == this.mRoom.name);
    ...
    doHauling()
    {
        ...
        if (this.mResources.length > 0)
        {
            ...
            {
                let res = this.mExtensionCreep.pickup(aResource);
            }
        }
