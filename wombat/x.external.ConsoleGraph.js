...
datasets:array
 	array of objects of the following format:
	{
		data:array -or- object
			...
			-or-
			Object with x:y value pairs. (x is the key, y is the value,
...
global.consoleGraph2 = function(datasets, opts={}) {
	let labelFormatter = (num) => {
		...
		return (num / size).toFixed(1).replace(/\.0$/,"") + letter;
	}
	...
	for (; (gridLineY - minYVal) < (yRange * 0.95); gridLineY += gridYStep) {
		...
		let y = opts.height - Math.floor(opts.height * ((gridLineY - minYVal) / yRange));
	...
	_.forEach(datasets, dataset => {
		...
		_.forEach(dataset.data, (yVal, xVal) => {
			let x = Math.floor((opts.width - 100) * ((xVal - minXVal) / xRange));
			let y = opts.height - Math.floor(opts.height * ((yVal - minYVal) / yRange));
			out += `${x},${y} `
		});
