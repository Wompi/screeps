...
var mod =
{
    ...
    runRemoteBuilder: function()
    {
        ...
        for ( var flag of myFlagNames)
        {
            if (_.isUndefined(aFlag) || flag.pos.getRangeTo(aCreep) < flag.pos.getRangeTo(aCreep))
            {
                aFlag = flag;
            }
        }

        // TODO: the grab room should be automated
        var aGrabRoom = Game.rooms['E66N49']


        var ignore = true;
        if (aCreep.pos.inRangeTo(aGrabRoom.mySpawns[0],3))
        {
            ignore = false;
        }


        if (_.isUndefined(aFlag))
        {
            var recyclePos = Room.RECYCLE_POSITIONS[aGrabRoom.name];

            if (_.isUndefined(recyclePos))
            {
                logERROR('ROOM '+aGrabRoom.name+' HAS NO RECYCLE POSITION - adjust this in Room.RECYCLE_POSITIONS!');
                return;
            }

            if (aCreep.pos.isEqualTo(recyclePos[0].x,recyclePos[0].y))
            {
                var result = aGrabRoom.mySpawns[0].recycleCreep(aCreep);
                logWARN('REMOTER BUILDER  '+aCreep.name+' is recycled  .. ' +ErrorSting(result));
            }
            else
            {
                var result = aCreep.moveTo(new RoomPosition(recyclePos[0].x,recyclePos[0].y,aGrabRoom.name),{ignoreCreeps: ignore});
                logWARN('REMOTER BUILDER  '+aCreep.name+' moves to recycle postion .. ' +ErrorSting(result));
            }
            return;
        }

        var isRemote = (aFlag.pos.roomName == aCreep.room.name);

        var myContainers = undefined;
        ...
        var myContainers = _.filter(aGrabRoom.myContainers,(a) =>
        {
            return a.store[RESOURCE_ENERGY] > 0;
        });
        var myLinks = _.filter(aGrabRoom.myLinks,(a) =>
        {
            return a.energy > 0;
        });
        var myGrabBoxes = myContainers.concat(myLinks);

        for (var aBox of myGrabBoxes)
        {
            if (_.isUndefined(aGrabBox) || aCreep.pos.getRangeTo(aBox) < aCreep.pos.getRangeTo(aGrabBox))
            {
                aGrabBox = aBox;
            }
        }
        // TODO: change this - could be stuck if all boxes are empty;
        if (_.isUndefined(aGrabBox)) return;
        ...
        {
            var result = aCreep.moveTo(aGrabBox,{ignoreCreeps: ignore, range: 1});
            logDEBUG('REMOTER BUILDER moves to grab box .. '+ErrorSting(result));
            result = aCreep.withdraw(aGrabBox,RESOURCE_ENERGY);
            logDEBUG('REMOTER BUILDER moves to grab from box .. '+ErrorSting(result));
        }
        else
        {
            if (aCreep.pos.isNearTo(aGrabBox) && aCreep.carry.energy < aCreep.energyCapacity)
            {
                result = aCreep.withdraw(aGrabBox,RESOURCE_ENERGY);
                logDEBUG('REMOTER BUILDER moves to grab from box till full .. '+ErrorSting(result));
            }
            else //if (aCreep.carry.energy == aCreep.energyCapacity || aCreep.pos.inRangeTo(aFlag,6))
            {
                var result = aCreep.moveTo(aFlag, {ignoreCreeps: ignore});
                logDEBUG('REMOTER BUILDER moves to flag .. '+ErrorSting(result));
                if (aCreep.room.name == aFlag.pos.roomName)
                {
                    var mySites = _.filter(Game.constructionSites,(a) =>
                    {
                        return a.pos.inRangeTo(aFlag,3);
                    })
                    ...
                    var result = aCreep.build(mySites[0]);
                    logDEBUG('REMOTER BUILDER builds something .. '+ErrorSting(result));
                }
            }
            // else
            // {
            //     var result = aCreep.moveTo(aGrabBox,{ignoreCreeps: true, range: 1});
            //     logDEBUG('REMOTER BUILDER moves to grab box .. '+ErrorSting(result));
            //     result = aCreep.withdraw(aGrabBox,RESOURCE_ENERGY);
            // }
        }
    ...
    spawnRemoteBuilder: function()
    {
        ...
        // logERROR('REMOTE SPAWN aFlag: '+JSON.stringify(aFlag));

        // TODO: until I find an easy way to get the distance to not diagonal rooms Iuse my main spawn here
        var myRooms = [Game.rooms['E66N49']];
        // var myRooms = _.filter(Game.rooms,(a) =>
        // {
        //     var aDist =  Game.map.getRoomLinearDistance(a.name,aFlag.pos.roomName);
        //     // logERROR('REMOTE: '+a.name+' - > '+aFlag.pos.roomName+' = '+aDist );
        //     return a.my &&  aDist == 1;
        // });
        if (myRooms.length == 0)
        {
        ...
        var aBody = undefined;
         _.forEach(myRooms, (a) =>
        {
            var derp = mySpawnCenter.getRemoteBuilder(a);
        ...
        // logERROR('REMOTE SPAWN aBody: '+JSON.stringify(aBody));
        // logERROR('REMOTE SPAWN spawnRoom: '+JSON.stringify(aRoom.name));


        var aName = 'R-Builder '+aRoom.name;
        var result = aRoom.mySpawns[0].createCreep(aBody,aName,{ role: 'remote builder'});
        ...
        {
            logERROR('REMOTE BUILDER something fishy with '+aName+' creep creation .. '+ErrorSting(result));
        }
    },
