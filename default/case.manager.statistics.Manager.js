var StatisticsManagerMemory = require('case.manager.statistics.Memory');

class StatisticsManager
{
    constructor()
    {
        this.mStatisticObjects = {};
        this.mMemory = new StatisticsManagerMemory(this);
    }


    registerStatisticsObject(pObject, pObjectType)
    {
        if (_.isUndefined(pObject)) return;
        if (_.isUndefined(pObjectType)) return;

        var value = this.mStatisticObjects[pObjectType];

        if (_.isUndefined(value))
        {
            this.mStatisticObjects[pObjectType] = [];
        }
        this.mStatisticObjects[pObjectType].push(pObject);
    }

    processManager()
    {
        logDERP('------- SOURCES -----------');
        this.processSourcesStats();
        logDERP('------- CONTROLLER -----------');
        this.processControllerStats();
        logDERP('------- GCL -----------');
        this.processGCLStats();
    }

    processGCLStats()
    {
        this.mMemory.saveGCL();
    }

    processControllerStats()
    {
        var myController = this.mStatisticObjects[ROOM_OBJECT_TYPE.controller];
        _.forEach(myController, (aController) =>
        {
            this.mMemory.saveController(aController);
        })
        this.mMemory.updateControllerTime();
    }


    processSourcesStats()
    {
        var mySources = this.mStatisticObjects[ROOM_OBJECT_TYPE.source];
        _.forEach(mySources, (aSource) =>
        {
            this.mMemory.saveSource(aSource);
        })
        this.mMemory.updateSourceTime();
    }


    printReport()
    {
        logERROR('STATISTICS MANAGER: --------------- STATS ---------------------------------');
        _.forEach(_.keys(this.mStatisticObjects),(a) =>
        {
            logERROR('STATS_MANAGER KEY: '+a+'\tCOUNT: '+this.mStatisticObjects[a].length);
        });
    }
