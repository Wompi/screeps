...
var mod =
{
    extend: function()
    {
        ...
        StructureSpawn.prototype.spawnTowerHaulerCreep = function()
        {
            if (this.spawning) return;
            var aBody = mySpawnCenter.getTowerHaulerCreep(this.room);
            if (_.isUndefined(aBody)) return;
            var filterCreep = _.filter(this.room.myCreeps,(a) => { return a.memory.role == 'towerHauler';});
            if (filterCreep.length > 0) return;

            var aCreep = filterCreep[0];

            //var result = this.createCreep([CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE],'Tower Hauler',{ role: 'towerHauler'});
            var aName = 'Supplyer '+this.room.name;
            var result = this.createCreep(aBody,aName,{ role: 'towerHauler'});
            if (typeof result === 'string')
            {
                logWARN('TOWER HAULER '+result+' is spawning .. '+ErrorSting(result));
                Game.creeps[result].init();
            }
            else
            {
                logERROR('TOWER HAULER something fishy with '+aName+' creep creation .. '+ErrorSting(result));
            }
        };

        StructureSpawn.prototype.spawnConstructionCreep = function()
        {
            if (this.spawning)
            {
                return;
            }
            var aBody = mySpawnCenter.getConstructorCreep(this.room);
            if (_.isUndefined(aBody))
            {
                return;
            }
            var filterCreep = _.filter(this.room.myCreeps,(a) => { return a.memory.role == 'constructor'});
            if (filterCreep.length > 0)
            {
                return;
            }
            if (this.room.myConstructionSites.length == 0)
            {
                return;
            }
            var myFlagNames = (Flag.findName(Flag.FLAG_COLOR.construction.primaryConstruction,this.room));
            var myAllFlags = myFlagNames.concat(Flag.findName(Flag.FLAG_COLOR.construction,this.room));
            if (myAllFlags.length == 0 )
            {
                logERROR('CONSTRUCTOR SPAWN '+this.room.name+' has construction sites but NO FLAGS .. place some to start!');
                return;
            }

            var result = this.createCreep(aBody,'Builder ' + this.room.name,{ role: 'constructor'});
            if (typeof result === 'string')
            {
            ...
            {
                logERROR('something fishy with constructor creep creation .. '+ErrorSting(result));
            }
        };

        StructureSpawn.prototype.spawnBlockerCreep = function()
        {
            var blockerCreep = _.filter(this.room.myCreeps,(aCreep) => { return aCreep.memory.role == 'blocker'});

            logERROR('Blocker: '+blockerCreep.name+' '+blockerCreep.spawning);
            if (blockerCreep.length == 0)
            {
        ...
        StructureSpawn.prototype.spawnCargoCreep = function()
        {
            var ENERGY_NEEDED = 500;
            var energyAvailable = this.room.energyAvailable;

            var myCargoCreep = undefined;
            for (var aID in Game.creeps)
            {
                var aCreep = Game.creeps[aID];
                if (aCreep.memory.role == 'cargo')
                {
                    myCargoCreep = aCreep;
                }
            }

            if (energyAvailable >= ENERGY_NEEDED && !this.spawning)
            {
                if (_.isUndefined(myCargoCreep))
                {
                    var result = this.createCreep([CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY],'Cargo',{role: 'cargo'});
                    var result = undefined;
                    if (result == 'Cargo')
                    {
                        Game.creeps['Cargo'].init();
                        logDEBUG('New cargo creep ... '+ErrorSting(result));
                    }
                    else
                    {
                        logWARN('something is fishy with cargo creep creation ERR: '+ErrorSting(result));
                    }
                }
            }
            return myCargoCreep;
